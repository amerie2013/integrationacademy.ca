...
export default function AssignmentSubmissionsPage() {
  ...
  async function load() {
    ...
    let rows: any[] = [];
    const withFiles = await supabase.from("submissions").select("id, student_id, content, grade, feedback, submitted_at, file_url, file_name").eq("assignment_id", id).order("submitted_at", { ascending: false });
    if (!withFiles.error) rows = withFiles.data ?? [];
    else { setFileCols(false); rows = (await supabase.from("submissions").select("id, student_id, content, grade, feedback, submitted_at").eq("assignment_id", id).order("submitted_at", { ascending: false })).data ?? []; }

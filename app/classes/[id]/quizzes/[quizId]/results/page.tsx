...
export default function ClassQuizResultsPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Access: the class's teacher, or an admin.
      ...
      // The quiz must belong to THIS class.
      const { data: q } = await supabase
        .from("quizzes")
        .select("title, class_id, passing_score")
        .eq("id", quizId)
        .single();
      if (!q || q.class_id !== classId) { setDenied(true); setLoading(false); return; }
  ...
  if (denied) return (<main><SiteHeader /><div style={{ padding: 48, color: "#64748b" }}>You don’t have access to this quiz’s results.</div></main>);

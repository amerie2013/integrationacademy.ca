...
const SRC = path.resolve("scripts/worksheets-alg1");
const PDF = path.resolve("scripts/_pdf");

const { data: course, error: cErr } = await db.from("courses").select("id").eq("code", "ALG1").single();
...
console.log(`\nPublished ${ok} worksheet(s) to ALG1.`);

// Seed (or re-seed) the MAP4C question bank from a unit module.
//   node scripts/bank-mpm2d/seed.mjs scripts/bank-mpm2d/u1.mjs
// The unit module default-exports an array of { code, gen } where gen() returns
// ~60 question objects. Each topic is matched to the MAP4C lesson whose title
...
const { data: course } = await db.from("courses").select("id").eq("code", "MAP4C").single();
if (!course) { console.error("MAP4C course not found."); process.exit(1); }
...
console.log(`\nSeeded ${total} MAP4C bank questions.`);

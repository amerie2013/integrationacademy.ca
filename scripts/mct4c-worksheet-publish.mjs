// Compile MCT4C worksheet .tex files with Tectonic and publish each as the
// course's worksheet PDF in Supabase.
//   node scripts/mct4c-worksheet-publish.mjs            # all .tex present
//   node scripts/mct4c-worksheet-publish.mjs 2.         # one unit, or 2.1 for one
import fs from "fs";
import os from "os";
...
const DIR = path.resolve("scripts/mct4c-worksheets");
...
const { data: course, error: cErr } = await db.from("courses").select("id").eq("code", "MCT4C").single();
if (cErr || !course) { console.error("MCT4C course not found:", cErr?.message); process.exit(1); }
...
for (const code of codes) {
  try {
    ...
    const remote = `${course.id}/mct4c_${code}_worksheet.pdf`;
...
console.log(`\nPublished ${ok}/${codes.length} MCT4C worksheet(s).`);

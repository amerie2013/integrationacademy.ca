// Seed (or re-seed) the MHF4U question bank from a unit module.
//   node scripts/bank-mhf4u/seed.mjs scripts/bank-mhf4u/u1.mjs
// The unit module default-exports an array of { code, gen } where gen() returns
// ~60 question objects. Each topic is matched to the MHF4U lesson whose title
...
const { data: course } = await db.from("courses").select("id").eq("code", "MHF4U").single();
if (!course) { console.error("MHF4U course not found."); process.exit(1); }
...
console.log(`\nSeeded ${total} MHF4U bank questions.`);

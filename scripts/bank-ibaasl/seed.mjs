// Seed (or re-seed) the IB AA SL question bank from a unit module.
//   node scripts/bank-ibaasl/seed.mjs scripts/bank-ibaasl/u1.mjs
// The unit module default-exports an array of { code, gen } where gen() returns
// question objects. Each topic is matched to the IBAASL lesson whose title
...
const { data: course } = await db.from("courses").select("id").eq("code", "IBAASL").single();
if (!course) { console.error("IBAASL course not found."); process.exit(1); }
...
console.log(`\nSeeded ${total} IBAASL bank questions.`);

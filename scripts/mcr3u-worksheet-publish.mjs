// Compile MCR3U worksheet .tex files with Tectonic and publish each as the
// course's worksheet PDF in Supabase.
//   node scripts/mcr3u-worksheet-publish.mjs            # all .tex present
//   node scripts/mcr3u-worksheet-publish.mjs 2.         # one unit, or 2.1 for one
import fs from "fs";
import os from "os";
...
const DIR = path.resolve("scripts/mcr3u-worksheets");
...
const { data: course, error: cErr } = await db.from("courses").select("id").eq("code", "MCR3U").single();
if (cErr || !course) { console.error("MCR3U course not found:", cErr?.message); process.exit(1); }
...
for (const code of codes) {
  try {
    ...
    const remote = `${course.id}/mcr3u_${code}_worksheet.pdf`;
...
console.log(`\nPublished ${ok}/${codes.length} MCR3U worksheet(s).`);

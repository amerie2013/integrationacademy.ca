// Compile MPM2D worksheet .tex files (scripts/mpm2d-worksheets/<code>.tex) with
// Tectonic and publish each as the course's worksheet PDF in Supabase.
//   node scripts/mpm2d-worksheet-publish.mjs            # all .tex present
//   node scripts/mpm2d-worksheet-publish.mjs 1.1        # just one
import fs from "fs";
import os from "os";
...
const { data: course, error: cErr } = await db.from("courses").select("id").eq("code", "MPM2D").single();
if (cErr || !course) { console.error("MPM2D course not found:", cErr?.message); process.exit(1); }

// derive a worksheet title from the matching lesson (e.g. "1.1 Review …" -> "Review …")
...
for (const code of codes) {
  try {
    ...
    const remote = `${course.id}/mpm2d_${code}_worksheet.pdf`;
...
console.log(`\nPublished ${ok}/${codes.length} MPM2D worksheet(s).`);

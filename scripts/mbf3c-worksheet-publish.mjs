// Compile MBF3C worksheet .tex files with Tectonic and publish each as the
// course's worksheet PDF in Supabase.
//   node scripts/mbf3c-worksheet-publish.mjs            # all .tex present
//   node scripts/mbf3c-worksheet-publish.mjs 2.         # one unit, or 2.1 for one
import fs from "fs";
import os from "os";
...
const DIR = path.resolve("scripts/mbf3c-worksheets");
...
const { data: course, error: cErr } = await db.from("courses").select("id").eq("code", "MBF3C").single();
if (cErr || !course) { console.error("MBF3C course not found:", cErr?.message); process.exit(1); }
...
for (const code of codes) {
  try {
    ...
    const remote = `${course.id}/mbf3c_${code}_worksheet.pdf`;
...
console.log(`\nPublished ${ok}/${codes.length} MBF3C worksheet(s).`);

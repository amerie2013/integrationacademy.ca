// Compile MFM2P worksheet .tex files (scripts/mfm2p-worksheets/<code>.tex) with
// Tectonic and publish each as the course's worksheet PDF in Supabase.
//   node scripts/mfm2p-worksheet-publish.mjs            # all .tex present
//   node scripts/mfm2p-worksheet-publish.mjs 3.         # one unit, or 3.1 for one
import fs from "fs";
import os from "os";
...
const DIR = path.resolve("scripts/mfm2p-worksheets");
...
const { data: course, error: cErr } = await db.from("courses").select("id").eq("code", "MFM2P").single();
if (cErr || !course) { console.error("MFM2P course not found:", cErr?.message); process.exit(1); }
...
for (const code of codes) {
  try {
    ...
    const remote = `${course.id}/mfm2p_${code}_worksheet.pdf`;
...
console.log(`\nPublished ${ok}/${codes.length} MFM2P worksheet(s).`);

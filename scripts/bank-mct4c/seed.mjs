// Seed (or re-seed) the MCT4C question bank from a unit module.
...
// ~60 question objects. Each topic is matched to the MCT4C lesson whose title
...
const { data: course } = await db.from("courses").select("id").eq("code", "MCT4C").single();
if (!course) { console.error("MCT4C course not found."); process.exit(1); }
...
console.log(`\nSeeded ${total} MCT4C bank questions.`);

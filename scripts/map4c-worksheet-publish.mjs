// Compile MAP4C worksheet .tex files with Tectonic and publish each as the
// course's worksheet PDF in Supabase.
//   node scripts/map4c-worksheet-publish.mjs            # all .tex present
//   node scripts/map4c-worksheet-publish.mjs 2.         # one unit, or 2.1 for one
import fs from "fs";
import os from "os";
...
const { data: course, error: cErr } = await db.from("courses").select("id").eq("code", "MAP4C").single();
if (cErr || !course) { console.error("MAP4C course not found:", cErr?.message); process.exit(1); }
...
for (const code of codes) {
  try {
    ...
    const remote = `${course.id}/map4c_${code}_worksheet.pdf`;
...
console.log(`\nPublished ${ok}/${codes.length} MAP4C worksheet(s).`);

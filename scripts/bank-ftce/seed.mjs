// Seed (or re-seed) the FTCE Mathematics 6-12 question bank from a unit module.
//   node scripts/bank-ftce/seed.mjs scripts/bank-ftce/u1.mjs
// The unit module default-exports an array of { code, gen } where gen() returns
// ~60 question objects. Each topic is matched to the ALG1 lesson whose title
// starts with that code; existing bank rows for that topic are replaced.
...
const { data: course } = await db.from("courses").select("id").eq("code", "FTCEMATH612").single();
if (!course) { console.error("FTCE course not found."); process.exit(1); }
...
console.log(`\nSeeded ${total} FTCE bank questions.`);

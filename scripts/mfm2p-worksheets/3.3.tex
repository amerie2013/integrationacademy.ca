...
\input{_preamble}
\fancyhead[R]{\footnotesize MFM2P \textperiodcentered{} 3.3}
\begin{document}

\begin{center}
{\LARGE\bfseries\color{titleblue}Worksheet 3.3 --- Solving by Elimination}\\[2pt]
...
\begin{learnbox}
\lh{The idea}\textbf{Elimination} adds or subtracts the equations to cancel a variable, scaling one first if needed.
\lh{The steps}Line up the equations, make a variable's coefficients opposite, add to eliminate it, then solve.
\lh{Choosing it}Elimination is handy when no variable is already isolated.
\end{learnbox}

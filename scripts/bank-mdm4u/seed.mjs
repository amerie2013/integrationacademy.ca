// Seed (or re-seed) the MDM4U question bank from a unit module.
...
// ~60 question objects. Each topic is matched to the MDM4U lesson whose title
...
const { data: course } = await db.from("courses").select("id").eq("code", "MDM4U").single();
if (!course) { console.error("MDM4U course not found."); process.exit(1); }
...
console.log(`\nSeeded ${total} MDM4U bank questions.`);

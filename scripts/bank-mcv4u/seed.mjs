// Seed (or re-seed) the MCV4U question bank from a unit module.
//   node scripts/bank-mcv4u/seed.mjs scripts/bank-mcv4u/u1.mjs
// The unit module default-exports an array of { code, gen } where gen() returns
// ~60 question objects. Each topic is matched to the MCV4U lesson whose title
...
const { data: course } = await db.from("courses").select("id").eq("code", "MCV4U").single();
if (!course) { console.error("MCV4U course not found."); process.exit(1); }
...
console.log(`\nSeeded ${total} MCV4U bank questions.`);

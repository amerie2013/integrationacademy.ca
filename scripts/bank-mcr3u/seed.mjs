// Seed (or re-seed) the MCR3U question bank from a unit module.
//   node scripts/bank-mcr3u/seed.mjs scripts/bank-mcr3u/u1.mjs
// The unit module default-exports an array of { code, gen } where gen() returns
// ~60 question objects. Each topic is matched to the MCR3U lesson whose title
...
const { data: course } = await db.from("courses").select("id").eq("code", "MCR3U").single();
if (!course) { console.error("MCR3U course not found."); process.exit(1); }
...
console.log(`\nSeeded ${total} MCR3U bank questions.`);

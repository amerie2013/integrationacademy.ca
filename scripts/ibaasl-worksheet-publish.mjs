// Compile IB AA SL worksheet .tex files with Tectonic and publish each as the
// course's worksheet PDF in Supabase.
//   node scripts/ibaasl-worksheet-publish.mjs            # all .tex present
//   node scripts/ibaasl-worksheet-publish.mjs 2.         # one unit, or 2.1 for one
import fs from "fs";
import os from "os";
...
const DIR = path.resolve("scripts/ibaasl-worksheets");
...
const { data: course, error: cErr } = await db.from("courses").select("id").eq("code", "IBAASL").single();
if (cErr || !course) { console.error("IBAASL course not found:", cErr?.message); process.exit(1); }
...
for (const code of codes) {
  try {
    ...
    const remote = `${course.id}/ibaasl_${code}_worksheet.pdf`;
...
console.log(`\nPublished ${ok}/${codes.length} IB AA SL worksheet(s).`);

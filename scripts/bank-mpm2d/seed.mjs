// Seed (or re-seed) the MPM2D question bank from a unit module.
...
// ~60 question objects. Each topic is matched to the MPM2D lesson whose title
...
const { data: course } = await db.from("courses").select("id").eq("code", "MPM2D").single();
if (!course) { console.error("MPM2D course not found."); process.exit(1); }
...
console.log(`\nSeeded ${total} MPM2D bank questions.`);

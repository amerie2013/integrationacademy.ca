// Compile MHF4U worksheet .tex files with Tectonic and publish each as the
// course's worksheet PDF in Supabase.
//   node scripts/mhf4u-worksheet-publish.mjs            # all .tex present
//   node scripts/mhf4u-worksheet-publish.mjs 2.         # one unit, or 2.1 for one
import fs from "fs";
import os from "os";
...
const DIR = path.resolve("scripts/mhf4u-worksheets");
...
const { data: course, error: cErr } = await db.from("courses").select("id").eq("code", "MHF4U").single();
if (cErr || !course) { console.error("MHF4U course not found:", cErr?.message); process.exit(1); }
...
for (const code of codes) {
  try {
    ...
    const remote = `${course.id}/mhf4u_${code}_worksheet.pdf`;
...
console.log(`\nPublished ${ok}/${codes.length} MHF4U worksheet(s).`);

// Seed (or re-seed) the MFM2P question bank from a unit module.
//   node scripts/bank-mfm2p/seed.mjs scripts/bank-mfm2p/reused.mjs
// The unit module default-exports an array of { code, gen } where gen() returns
// ~60 question objects. Each topic is matched to the MFM2P lesson whose title
...
const { data: course } = await db.from("courses").select("id").eq("code", "MFM2P").single();
if (!course) { console.error("MFM2P course not found."); process.exit(1); }
...
console.log(`\nSeeded ${total} MFM2P bank questions.`);

...
const SRC = path.resolve("scripts/worksheets-ftce");
const PDF = path.resolve("scripts/_pdf");

const { data: course, error: cErr } = await db.from("courses").select("id").eq("code", "FTCEMATH612").single();
if (cErr || !course) { console.error("FTCE course not found:", cErr?.message); process.exit(1); }
...
console.log(`\nPublished ${ok} worksheet(s) to FTCEMATH612.`);

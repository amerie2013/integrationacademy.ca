// Seed (or re-seed) the MCF3M question bank from a unit module.
//   node scripts/bank-mcf3m/seed.mjs scripts/bank-mcf3m/u1.mjs
// The unit module default-exports an array of { code, gen } where gen() returns
// ~60 question objects. Each topic is matched to the MCF3M lesson whose title
...
const { data: course } = await db.from("courses").select("id").eq("code", "MCF3M").single();
if (!course) { console.error("MCF3M course not found."); process.exit(1); }
...
console.log(`\nSeeded ${total} MCF3M bank questions.`);

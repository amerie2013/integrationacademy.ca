// Compile MCV4U worksheet .tex files with Tectonic and publish each as the
// course's worksheet PDF in Supabase.
//   node scripts/mcv4u-worksheet-publish.mjs            # all .tex present
//   node scripts/mcv4u-worksheet-publish.mjs 2.         # one unit, or 2.1 for one
import fs from "fs";
import os from "os";
...
const DIR = path.resolve("scripts/mcv4u-worksheets");
...
const { data: course, error: cErr } = await db.from("courses").select("id").eq("code", "MCV4U").single();
if (cErr || !course) { console.error("MCV4U course not found:", cErr?.message); process.exit(1); }
...
for (const code of codes) {
  try {
    ...
    const remote = `${course.id}/mcv4u_${code}_worksheet.pdf`;
...
console.log(`\nPublished ${ok}/${codes.length} MCV4U worksheet(s).`);
